...
LIGHTNING_FLASH_COLOR = (220, 228, 242)
LIGHTNING_FLASH_MAX_ALPHA = 145
...
def draw_lightning_flash(screen, weather: WeatherSystem) -> None:
    """Draw the brief atmospheric flash generated by a thunderstorm."""
    if weather.lightning_intensity <= 0.0:
        return
    import pygame

    flash = _reusable_alpha_surface(pygame, "lightning_flash", screen.get_size())
    flash.fill(
        (*LIGHTNING_FLASH_COLOR, round(LIGHTNING_FLASH_MAX_ALPHA * weather.lightning_intensity))
    )
