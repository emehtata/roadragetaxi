...
def test_tile_streaming_loads_missing_tiles_in_background():
    ...
    class TileCache:
        ...
        def preload_tile(self, tile, bbox):
            self.calls.append((tile, bbox))
            future = Future()
            future.set_result(MapData([], [], [], [], [], tile_bbox(tile)))
            return future
    ...
    assert manager.integrate_completed_tiles() == 9
    assert len(cache.calls) == 9
...
def test_startup_world_is_registered_and_trimmed_to_active_tiles():
    ...
    assert manager.loaded_tiles == {TileCoord(0, 0)}
